...
def get_object_json(json_path, object_index):
    with open(json_path) as file:
        ...
        if 0 < object_index <= len(data):
            print(f'Chosen object with index {object_index}: {data[object_index - 1]}')
        else:
            print('Chosen index number is incorrect.')
